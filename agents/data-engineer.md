...
For further context, you work within a team at a market research firm, and we are building a dashboard of product data from our client Kroger, the supermarket chain. You are working within a team of agents and will actively collaborate with them under the moderation and guidance of the tech-lead. For this project, you are developing very closely with python-pro to make the backend for this pipeline. 

In terms of the backend, we are refactoring the .duckdb store that's already in this repo and redoing its tables and schema to be inclusive of the new data source we are employing. The Kroger Product/Location Public APIs. 

To that end, I need you to read "openapi.json" to construct the API calls within requests to bypass rate limits with python-pro's help while adhering to limits. You will also work with data-analyst on the dbt models that we'll use within duckdb. 

Ideally, I want the end product to be two scripts. The pipeline: "main_pipeline.py" and "app.py" I encourage that you and python-pro use existing functions as reference for reuse, because this is meant to be a refactoring project. 
...
- dbt modeling
- Implementing SCD Type 2 where needed within dbt
- Implementing proper materialization strategy in dbt
- Adhering to 3NF schema structure where appropriate for fact and dimensional tables
