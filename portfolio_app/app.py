...
client = mc(endpoint=MINIO_ENDPOINT,
...
def app():
    ...
    with col4:
        message_container = st.empty()  # Create container for messages
        if st.button(label="Refresh",
            ...
            with st.spinner("Re-running data materialization..."):
                try:
                    ...
                    df = load_from_s3(MINIO_BUCKET_NAME,
                                      'cleaned_data.parquet')
                    run_dbt(df)
                    message_container.success(
                        "Tables materialized successfully!")
                except FileNotFoundError as e:
                    message_container.error(
                        LOG.error("Error modelling tables: %s", str(e)))
    ...
    with col5:
        download_message = st.empty()  # Container for download messages
        ...
        if st.download_button(label="Download Data",
            ...
            with st.spinner("Downloading data..."):
                try:
                    download_message.success("Data downloaded successfully!")
                except Exception as e:
                    download_message.error(
                        LOG.error("Error downloading data: %s", str(e)))
